...
export default function WriterAvailableOrderTable({ availableOrdersData }) {
  const router = useRouter();
  const pageName = router.asPath.split('/')[2];
  console.log(availableOrdersData);
  return (
            ...
            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <table className="min-w-full divide-y divide-gray-300">
                ...
                <tbody className="divide-y divide-gray-200 dark:bg-[#273142] ">
                  {availableOrdersData.map((item, idx) => (
                    <tr key={idx}>
                      <td className="whitespace-nowrap py-3 pl-4 text-sm dark:text-gray-400">
                        {item.ID}
                      </td>
